			    ...
			    pappl_printer_t *printer)
{
	...
	snprintf(command, sizeof(command), "{D%04d,%04d,%04d,%04d|}\n",
		 label_pitch, width, height, roll_width);

	if (job)
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "Sending label size command: %s", command);
	else if (printer)
		      ...
		      pappl_job_t *job, pappl_printer_t *printer)
{
	...
	snprintf(command, sizeof(command), "{T%c%c%c%c%c|}\n", sensor_char,
		 cut_char, mode_char, speed_char, ribbon_char);

	if (job)
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "Sending feed command: %s", command);
	else if (printer)
		papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
				"Sending feed command: %s", command);
				 ...
				 pappl_job_t *job, pappl_printer_t *printer)
{
	...
	snprintf(command, sizeof(command), "{AX;%c%03d,%c%03d,%c%02d|}\n",
	...
	if (job)
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "Sending position adjustment command: %s", command);
	else if (printer)
		papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
				"Sending position adjustment command: %s",
				command);
				 ...
				 pappl_printer_t *printer)
{
	...
	snprintf(command, sizeof(command), "{AY;%c%02d,%c|}\n",
		 (darkness >= 0) ? '+' : '-', abs(darkness), type_char);

	if (job)
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "Sending darkness adjustment command: %s", command);
	else if (printer)
		papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
				"Sending darkness adjustment command: %s",
				command);
			      ...
			      pappl_printer_t *printer)
{
	if (job)
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "Sending clear image buffer command: {C|}");
	else if (printer)
		papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
				"Sending clear image buffer command: {C|}");
				 ...
				 pappl_job_t *job, pappl_printer_t *printer)
{
	...
	snprintf(command, sizeof(command), "{SG;%04d,%05d,%04u,%05u,%d,",
		 x_origin, y_origin, width, height, gmode);

	if (job)
		papplLogJob(
			job, PAPPL_LOGLEVEL_DEBUG,
			"Sending graphic command header (width, height, mode): %s",
			command);
	else if (printer)
		papplLogPrinter(
			     ...
			     pappl_printer_t *printer)
{
	...
	snprintf(command, sizeof(command), "{XS;I,%04d,%03d%c%c%c%c%c%c|}\n",
		 copies, cut_interval, sensor_char, mode_char, speed_char,
		 ribbon_char, rotation, response);

	if (job)
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "Sending issue label command: %s", command);
	else if (printer)
		papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
				"Sending issue label command: %s", command);
		      ...
		      pappl_printer_t *printer)
{
	...
	snprintf(command, sizeof(command), "{LC;%04d,%04d,%04d,%04d,%d,%d|}\n",
		 x1, y1, x2, y2, type, line_width);

	if (job)
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "Sending LC command for box: %s", command);
	else if (printer)
		papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
				"Sending LC command for box: %s", command);
