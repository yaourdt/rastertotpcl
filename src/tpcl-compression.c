...
tpcl_compbuf_t *tpcl_compbuf_create(unsigned int line_bytes, pappl_job_t *job)
{
	tpcl_compbuf_t *compbuf =
		(tpcl_compbuf_t *)calloc(1, sizeof(tpcl_compbuf_t));

	if (!compbuf) {
		if (job)
			papplLogJob(
				job, PAPPL_LOGLEVEL_ERROR,
				"Failed to allocate compression buffer structure");
		return NULL;
	}
	...
	if (!compbuf->last_buffer || !compbuf->comp_buffer) {
		if (job)
			papplLogJob(
				job, PAPPL_LOGLEVEL_ERROR,
				"Failed to allocate TOPIX compression buffers");
	...
	if (job)
		papplLogJob(
			...
			"TOPIX buffers allocated: line=%u bytes, comp=65535 bytes",
			line_bytes);
...
{
	if (compbuf) {
		if (compbuf->last_buffer)
			free(compbuf->last_buffer);
		if (compbuf->comp_buffer)
			free(compbuf->comp_buffer);
		free(compbuf);
	}
}
			      ...
			      const unsigned char *line_data)
{
	...
	unsigned char line[8][9][9] = {
		{ { 0 } }
	}; // Current line (L1 x L2 x L3)
	...
	for (l1 = 0; l1 <= 7 && i < width; l1++) {
		...
		for (l2 = 1; l2 <= 8 && i < width; l2++) {
			cl3 = 0;
			for (l3 = 1; l3 <= 8 && i < width; l3++,
			    i++) // Careful, this loop increments two variables!
			{
			 ...
			 unsigned int resolution, int gmode)
{
	...
	unsigned int printer_resolution = (resolution == 203) ? 300 :
								resolution;

	// Send SG command with compressed data
	snprintf(
		command, sizeof(command), "{SG;0000,%05d,%04u,%05u,%d,",
		// x_origin in 0.1mm
	...
	bytes_written += papplDeviceWrite(device, &belen,
					  2); // Total length of graphic data
	bytes_written += papplDeviceWrite(device, compbuf->comp_buffer,
					  len); // Compressed data
