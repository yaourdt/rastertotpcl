			       ...
			       ipp_t **driver_attrs)
{
	...
	tpcl_add_vendor_int_option(driver_data, driver_attrs, "label-gap", 0,
				   200, 50);

	// Roll margin in units of 0.1mm (width difference between backing paper and label)
	tpcl_add_vendor_int_option(driver_data, driver_attrs, "roll-margin", 0,
				   300, 10);

	// Sensor type for label detection
	tpcl_add_vendor_str_option(driver_data, driver_attrs, "sensor-type", 5,
				   (const char *[]){ "none", "reflective",
						     "transmissive",
	...
	tpcl_add_vendor_str_option(driver_data, driver_attrs, "label-cut", 2,
				   (const char *[]){ "non-cut", "cut" },
				   "non-cut");

	// Cut interval (number of labels before cutting, 0=no cut, 1-100)
	tpcl_add_vendor_int_option(driver_data, driver_attrs, "cut-interval", 0,
				   100, 0);

	// Feed mode selection
	tpcl_add_vendor_str_option(
		driver_data, driver_attrs, "feed-mode", 4,
		(const char *[]){ "batch", "strip-backfeed-sensor",
				  "strip-backfeed-no-sensor", "partial-cut" },
		"batch");

	// Feed on label size change?
	tpcl_add_vendor_str_option(driver_data, driver_attrs,
				   "feed-on-label-size-change", 2,
				   (const char *[]){ "yes", "no" }, "yes");

	// Graphics mode selection
	tpcl_add_vendor_str_option(
		driver_data, driver_attrs, "graphics-mode", 5,
		(const char *[]){ "nibble-and", "hex-and", "topix", "nibble-or",
				  "hex-or" },
		"topix");

	// Dithering algorithm selection
	tpcl_add_vendor_str_option(
		driver_data, driver_attrs, "dithering-algorithm", 3,
		(const char *[]){ "threshold", "bayer", "clustered" },
		"threshold");

	// Dithering algorithm selection for photo content
	tpcl_add_vendor_str_option(
		driver_data, driver_attrs, "dithering-algorithm-photo", 3,
		(const char *[]){ "threshold", "bayer", "clustered" },
		"threshold");

	// Dithering threshold level (0-255, only used with 'threshold' algorithm)
	tpcl_add_vendor_int_option(driver_data, driver_attrs,
			      ...
			      ipp_t **driver_attrs)
{
	...
	const char *dither_algo = tpcl_get_str_option(
		*driver_attrs, "dithering-algorithm", "threshold", NULL, NULL);

	if (strcmp(dither_algo, "bayer") == 0) {
		dither_bayer16(
			driver_data
				->gdither); // Dithering for 'auto', 'text', and 'graphic'
	} else if (strcmp(dither_algo, "clustered") == 0) {
		dither_clustered16(driver_data->gdither);
	} else {
		int dither_threshold = tpcl_get_int_option(
			*driver_attrs, "dithering-threshold", 128, NULL, NULL);
		dither_threshold16(driver_data->gdither,
				   (unsigned char)dither_threshold);
	}

	// Configure dithering for photo content based on separate photo algorithm option
	const char *dither_algo_photo =
		tpcl_get_str_option(*driver_attrs, "dithering-algorithm-photo",
				    "threshold", NULL, NULL);
	...
	if (strcmp(dither_algo_photo, "bayer") == 0) {
		...
		int dither_threshold = tpcl_get_int_option(
			*driver_attrs, "dithering-threshold", 128, NULL, NULL);
		dither_threshold16(driver_data->pdither,
				   (unsigned char)dither_threshold);
	}
	...
	driver_data->format =
		"application/vnd.toshiba-tpcl"; // Native file format
	...
	driver_data->output_face_up =
		true; // Does output media come out face-up?
	driver_data->orient_default =
		IPP_ORIENT_PORTRAIT; // Default orientation
	driver_data
		->color_supported = // Highest supported color mode advertised via IPP
	...
	driver_data->content_default =
		PAPPL_CONTENT_AUTO; // Optimize for vector graphics or image content
	driver_data->quality_default =
		IPP_QUALITY_NORMAL; // Default print quality
	driver_data->scaling_default =
		PAPPL_SCALING_AUTO; // Default print scaling
	driver_data
		->raster_types = // Supported color schemes by our driver callback
	...
	driver_data->duplex =
		PAPPL_DUPLEX_NONE; // Duplex printing modes supported
	driver_data->sides_supported =
		PAPPL_SIDES_ONE_SIDED; // IPP "sides" bit values
	driver_data->sides_default =
		PAPPL_SIDES_ONE_SIDED; // IPP "sides" bit values for default
	driver_data->finishings =
		PAPPL_FINISHINGS_NONE; // Supported finishings such as punch or staple
	driver_data->num_bin = 0; // Number of output bins
	driver_data
		->identify_supported = // Supported identify actions (we actually feed a blank label)
		PAPPL_IDENTIFY_ACTIONS_SOUND;
	driver_data
		->identify_default = // Default identification action (we actually feed a blank label)
		PAPPL_IDENTIFY_ACTIONS_SOUND;
	...
	driver_data->tear_offset_supported[0] =
		0; // Min offset when in mode for tearing labels
	driver_data->tear_offset_supported[1] =
		180; // Max offset when in mode for tearing labels
	driver_data->tear_offset_configured =
		0; // Default offset when in mode for tearing labels
			     ...
			     ipp_t **driver_attrs)
{
	for (int i = 0; i < tpcl_drivers_count; i++) {
		if (!strcmp(driver_name, tpcl_drivers[i].name)) {
			int x_mm, y_mm;
			char roll_min[PAPPL_MAX_MEDIA],
				roll_max[PAPPL_MAX_MEDIA];

			// Device name
			strncpy(driver_data->make_and_model,
				tpcl_drivers[i].description, 127);
			papplLog(system, PAPPL_LOGLEVEL_DEBUG,
				 "Driver '%s' loaded from table position %d",
				 tpcl_drivers[i].name, i);

			// Available printer resolutions
			driver_data->num_resolution =
				0; // Number of printer resolutions
			if (tpcl_printer_properties[i].resolution_203) {
				driver_data->x_resolution[0] =
					203; // Horizontal printer resolutions
				driver_data->y_resolution[0] =
					203; // Vertical printer resolutions
				driver_data->x_default =
					203; // Default horizontal resolution
				driver_data->y_default =
					203; // Default vertical resolution
				driver_data->num_resolution++;
			}
			if (tpcl_printer_properties[i].resolution_300) {
				driver_data->x_resolution
					[driver_data->num_resolution] = 300;
				driver_data->y_resolution
					[driver_data->num_resolution] = 300;
			...
			papplLog(
				system, PAPPL_LOGLEVEL_DEBUG,
				"Resolution settings: num_resolution=%d, x_default=%d, y_default=%d",
				driver_data->num_resolution,
				driver_data->x_default, driver_data->y_default);
			if (driver_data->num_resolution == 0) {
				papplLog(
					system, PAPPL_LOGLEVEL_ERROR,
					"No resolution configured for driver '%s'",
					driver_name);
				return false;
			}
			...
			tpcl_add_vendor_int_option(
				driver_data, driver_attrs, "print-speed",
				tpcl_printer_properties[i].print_speeds[0],
				tpcl_printer_properties[i].print_speeds[2],
				tpcl_printer_properties[i].print_speeds[1]);
			papplLog(
				system, PAPPL_LOGLEVEL_DEBUG,
				"Print speed settings: min=%d, default=%d, max=%d",
				tpcl_printer_properties[i].print_speeds[0],
				tpcl_printer_properties[i].print_speeds[1],
				tpcl_printer_properties[i].print_speeds[2]);
			...
			// Minimum label size
			x_mm = (int)ceil(
				(tpcl_printer_properties[i].print_min_width /
				 POINTS_PER_INCH) *
				MM_PER_INCH);
			y_mm = (int)ceil(
				(tpcl_printer_properties[i].print_min_height /
				 POINTS_PER_INCH) *
				MM_PER_INCH);
			snprintf(roll_min, PAPPL_MAX_MEDIA, "roll_min_%dx%dmm",
				 x_mm, y_mm);
			driver_data->media[0] = strdup(roll_min);

			// Maximum label size
			x_mm = (int)floor(
				(tpcl_printer_properties[i].print_max_width /
				 POINTS_PER_INCH) *
				MM_PER_INCH);
			y_mm = (int)floor(
				(tpcl_printer_properties[i].print_max_height /
				 POINTS_PER_INCH) *
				MM_PER_INCH);
			snprintf(roll_max, PAPPL_MAX_MEDIA, "roll_max_%dx%dmm",
				 x_mm, y_mm);
			driver_data->media[1] = strdup(roll_max);
			papplLog(system, PAPPL_LOGLEVEL_DEBUG,
				 "Roll media dimensions: min=%s, max=%s",
				 driver_data->media[0], driver_data->media[1]);

			// Available media sources
			driver_data->num_source =
				1; // Number of media sources (trays/rolls)
			driver_data->source[0] = "main-roll"; // Media sources
			papplCopyString(driver_data->media_ready[0].source,
					driver_data->source[0], 63);

			// Available media types
			driver_data->num_type = 1; // Number of media types
			driver_data->type[0] = "direct-thermal"; // Media types
			papplCopyString(driver_data->media_ready[0].type,
					driver_data->type[0], 63);

			if (tpcl_printer_properties[i].thermal_transfer) {
				driver_data->type[driver_data->num_type] =
					"thermal-transfer";
				driver_data->num_type++;
			}

			if (tpcl_printer_properties[i]
				    .thermal_transfer_with_ribbon) {
				driver_data->type[driver_data->num_type] =
			...
			papplLog(
				system, PAPPL_LOGLEVEL_DEBUG,
				"Media type settings: num_type=%d, thermal_transfer=%d, thermal_transfer_with_ribbon=%d",
				driver_data->num_type,
				tpcl_printer_properties[i].thermal_transfer,
				tpcl_printer_properties[i]
					.thermal_transfer_with_ribbon);

			// Fill out ready media, by default we are not setting margins
			driver_data->borderless =
				false; // Offer the option to toggle borderless in the UI. Makes no sense, we are always borderless
			driver_data->left_right =
				0; // Left and right margins in hundredths of millimeters
			driver_data->bottom_top =
				0; // Bottom and top margins in hundredths of millimeters
			driver_data->media_ready[0].top_margin =
				0; // Top margin in hundredths of millimeters
			driver_data->media_ready[0].bottom_margin =
				0; // Bottom margin in hundredths of millimeters
			driver_data->media_ready[0].left_margin =
				0; // Left margin in hundredths of millimeters
			driver_data->media_ready[0].right_margin =
				0; // Right margin in hundredths of millimeters

			// Fill out ready media, we assume a default label of size 80x200mm to be loaded
			driver_data->media_ready[0].tracking =
				0; // IPP media tracking type, we have no tracking
			driver_data->media_ready[0].size_width =
				8000; // Width in hundredths of millimeters
			driver_data->media_ready[0].size_length =
				20000; // Height in hundredths of millimeters
			papplCopyString(driver_data->media_ready[0].size_name,
					"oe_toshiba_80x200mm", 63);
			papplLog(
				system, PAPPL_LOGLEVEL_DEBUG,
				"Media ready settings: size_name=%s, width=%d (0.01mm), length=%d (0.01mm)",
				driver_data->media_ready[0].size_name,
				driver_data->media_ready[0].size_width,
				driver_data->media_ready[0].size_length);

			// Fill out ready media, by default there are no offsets
			driver_data->left_offset_supported[0] =
				0; // Minimum left printing offset supported
			driver_data->left_offset_supported[1] =
				0; // Maximum left printing offset supported
			driver_data->media_ready[0].left_offset =
				0; // Default left offset in hundredths of millimeters
			driver_data->top_offset_supported[0] =
				0; // Minimum top printing offset supported
			driver_data->top_offset_supported[1] =
				0; // Maximum top printing offset supported
			driver_data->media_ready[0].top_offset =
				0; // Default top offset in hundredths of millimeters

			// Printer darkness (workaround due to PAPPL web interface limitations)
			driver_data->darkness_supported = 0; // Printer darkness
			driver_data->darkness_default =
				0; // Default darkness adjustment at printer power-on
			driver_data->darkness_configured =
				0; // Currently configured printing darkness
			tpcl_add_vendor_int_option(driver_data, driver_attrs,
						   "print-darkness", -10, 10,
						   0);

			// Default media
			driver_data->media_default =
				driver_data->media_ready[0];
			return true;
		}
	}
	...
	papplLog(system, PAPPL_LOGLEVEL_ERROR,
		 "Driver '%s' not found in driver table", driver_name);
	return false;
}
