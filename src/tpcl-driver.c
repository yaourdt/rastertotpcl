...
bool tpcl_driver_cb(pappl_system_t *system, const char *driver_name,
		    const char *device_uri, const char *device_id,
		    pappl_pr_driver_data_t *driver_data, ipp_t **driver_attrs,
		    void *data);

bool tpcl_status_cb(pappl_printer_t *printer);

void tpcl_identify_cb(pappl_printer_t *printer,
		      pappl_identify_actions_t actions, const char *message);

bool tpcl_print_cb(pappl_job_t *job, pappl_pr_options_t *options,
		   pappl_device_t *device);

bool tpcl_rstartjob_cb(pappl_job_t *job, pappl_pr_options_t *options,
		       pappl_device_t *device);

bool tpcl_rstartpage_cb(pappl_job_t *job, pappl_pr_options_t *options,
			pappl_device_t *device, unsigned page);

bool tpcl_rwriteline_cb(pappl_job_t *job, pappl_pr_options_t *options,
			pappl_device_t *device, unsigned y,
			const unsigned char *line);

bool tpcl_rendpage_cb(pappl_job_t *job, pappl_pr_options_t *options,
		      pappl_device_t *device, unsigned page);

bool tpcl_rendjob_cb(pappl_job_t *job, pappl_pr_options_t *options,
		     pappl_device_t *device);

const char *tpcl_testpage_cb(pappl_printer_t *printer, char *buffer,
			     size_t bufsize);

void tpcl_delete_cb(pappl_printer_t *printer, pappl_pr_driver_data_t *data);

/*
 * Support functions
 */

static void tpcl_free_job_buffers(pappl_job_t *job, tpcl_job_t *tpcl_job);
...
bool tpcl_driver_cb(pappl_system_t *system, // System config
...
)
{
	...
	driver_data->identify_cb =
		tpcl_identify_cb; // Identify-Printer callback (feed one blank label)
	driver_data->printfile_cb = tpcl_print_cb; // Print (raw) file callback
	driver_data->rstartjob_cb =
		tpcl_rstartjob_cb; // Start raster job callback
	driver_data->rstartpage_cb =
		tpcl_rstartpage_cb; // Start raster page callback
	driver_data->rwriteline_cb =
		tpcl_rwriteline_cb; // Write raster line callback
	...
	if (!tpcl_setup_driver_model(system, driver_name, driver_data,
				     driver_attrs))
		return false;
...
bool tpcl_status_cb(pappl_printer_t *printer)
{
	...
	if (!device) {
		papplLogPrinter(
			printer, PAPPL_LOGLEVEL_ERROR,
			"Failed to open device connection for status query");
		return printer_ready;
	}

	// Send status query command
	papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
			"Status query sent, waiting for response...");
	...
	if (bytes < 0) {
		papplLogPrinter(
			printer, PAPPL_LOGLEVEL_ERROR,
			"Error reading status response (error code: %ld)",
			(long)bytes);
		papplPrinterCloseDevice(printer);
		return printer_ready;
	} else if (bytes == 0) {
		papplLogPrinter(printer, PAPPL_LOGLEVEL_ERROR,
				"Timeout waiting for status response (%dms)",
				poll_attempts);
		papplPrinterCloseDevice(printer);
		return printer_ready;
	}
	...
	if (bytes >= 13 && status[0] == 0x01 && status[1] == 0x02) {
		...
		status_code[2] = '\0';
		papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
				"Status response: '%s' after %dms", status_code,
				poll_attempts);

		// Check status code against documented values: "00"=ready, "02"=operating, "40"=print succeeded, "41"=feed succeeded
		if (strcmp(status_code, "00") == 0 ||
		    strcmp(status_code, "02") == 0 ||
		    strcmp(status_code, "40") == 0 ||
		    strcmp(status_code, "41") == 0) {
			printer_ready = true;
			papplLogPrinter(printer, PAPPL_LOGLEVEL_INFO,
					"Printer ready (status: %s)",
					status_code);
			...
			if (strcmp(status_code, "01") == 0) {
				...
				papplPrinterSetReasons(
					printer, PAPPL_PREASON_COVER_OPEN,
					PAPPL_PREASON_DEVICE_STATUS);
				...
				papplLogPrinter(printer, PAPPL_LOGLEVEL_ERROR,
						"Paper jam");
				papplPrinterSetReasons(
					printer, PAPPL_PREASON_MEDIA_JAM,
					PAPPL_PREASON_DEVICE_STATUS);
				...
				papplPrinterSetReasons(
					printer, PAPPL_PREASON_MEDIA_EMPTY,
					PAPPL_PREASON_DEVICE_STATUS);
			} else if (strcmp(status_code, "15") == 0) {
				papplLogPrinter(
					printer, PAPPL_LOGLEVEL_ERROR,
					"Feed attempt while cover open");
				papplPrinterSetReasons(
					printer, PAPPL_PREASON_COVER_OPEN,
					PAPPL_PREASON_DEVICE_STATUS);
			} else if (strcmp(status_code, "16") == 0) {
				papplLogPrinter(printer, PAPPL_LOGLEVEL_ERROR,
						"Steeping motor overheat");
				papplPrinterSetReasons(
					printer, PAPPL_PREASON_OTHER,
					PAPPL_PREASON_DEVICE_STATUS);
				...
				papplLogPrinter(
					printer, PAPPL_LOGLEVEL_ERROR,
					"Print succeeded. The label has run out");
				papplPrinterSetReasons(
					printer, PAPPL_PREASON_MEDIA_EMPTY,
					PAPPL_PREASON_DEVICE_STATUS);
				...
				papplLogPrinter(printer, PAPPL_LOGLEVEL_ERROR,
						"SD card full");
				papplPrinterSetReasons(
					printer, PAPPL_PREASON_SPOOL_AREA_FULL,
					PAPPL_PREASON_DEVICE_STATUS);
			} else if (strcmp(status_code, "55") == 0) {
				papplLogPrinter(
					printer, PAPPL_LOGLEVEL_ERROR,
					"PC command mode / initialize SD / EEPROM error");
				papplPrinterSetReasons(
					printer, PAPPL_PREASON_OTHER,
					PAPPL_PREASON_DEVICE_STATUS);
			} else {
		...
		papplLogPrinter(
			printer, PAPPL_LOGLEVEL_ERROR,
			"Invalid status response format (received %ld bytes)",
			(long)bytes);
	}
...
{
	papplLogPrinter(printer, PAPPL_LOGLEVEL_INFO,
			"Printer identification triggered: Eject one label");
	...
	if (!device) {
		papplLogPrinter(
			printer, PAPPL_LOGLEVEL_ERROR,
			"Failed to open device connection for printer identification");
		return;
	}
	...
	if (!papplPrinterGetDriverData(printer, &driver_data)) {
		papplLogPrinter(
			printer, PAPPL_LOGLEVEL_ERROR,
			"Failed to get driver data for printer identification");
		papplPrinterCloseDevice(printer);
		return;
	}

	// Calculate dimensions from media_default (convert hundredths of mm to tenths of mm)
	print_width = driver_data.media_default.size_width /
		      10; // Effective print width
	print_height = driver_data.media_default.size_length /
		       10; // Effective print height

	// Request printer IPP attributes
	ipp_t *printer_attrs = papplPrinterGetDriverAttributes(printer);
	if (!printer_attrs) {
		papplLogPrinter(printer, PAPPL_LOGLEVEL_ERROR,
				"Failed to get printer attributes");
		papplPrinterCloseDevice(printer);
		return;
	}

	// Get label dimensions using helper function
	tpcl_get_label_dimensions(printer_attrs, print_width, print_height,
				  &label_pitch, &roll_width, NULL, printer);
	...
	if (label_pitch > max_pitch) {
		papplLogPrinter(
			printer, PAPPL_LOGLEVEL_ERROR,
			"Label pitch %d (0.1mm) exceeds maximum %d (0.1mm) for %ddpi resolution",
			label_pitch, max_pitch, driver_data.y_default);
	...
	if (print_height > max_height) {
		papplLogPrinter(
			printer, PAPPL_LOGLEVEL_ERROR,
			"Print height %d (0.1mm) exceeds maximum %d (0.1mm) for %ddpi resolution",
			print_height, max_height, driver_data.y_default);
		ippDelete(printer_attrs);
		papplPrinterCloseDevice(printer);
		return;
	}
	...
	tpcl_cmd_label_size(device, label_pitch, print_width, print_height,
			    roll_width, NULL, printer);
	...
	const char *sensor_type = tpcl_get_str_option(
		printer_attrs, "sensor-type", "transmissive", NULL, printer);
	char sensor_char = tpcl_map_sensor_type(sensor_type);

	// Cut selection (0=non-cut, 1=cut)
	const char *cut_type = tpcl_get_str_option(printer_attrs, "label-cut",
						   "non-cut", NULL, printer);
	...
	const char *feed_mode = tpcl_get_str_option(printer_attrs, "feed-mode",
						    "batch", NULL, printer);
	char feed_mode_char = tpcl_map_feed_mode(feed_mode);

	// Feed speed (retrieve from vendor option with fallback to 3)
	int print_speed = tpcl_get_int_option(printer_attrs, "print-speed", 3,
					      NULL, printer);
	...
	if (media_type) {
		if (strcmp(media_type, "thermal-transfer-ribbon-saving") == 0)
			ribbon_char = '1';
		else if (strcmp(media_type,
				"thermal-transfer-no-ribbon-saving") == 0)
			ribbon_char = '2';
	}

	// Send feed command
	tpcl_cmd_feed(device, sensor_char, cut_char, feed_mode_char, speed_char,
		      ribbon_char, NULL, printer);
		   ...
		   pappl_device_t *device)
{
	papplLogJob(job, PAPPL_LOGLEVEL_INFO,
		    "Starting raw TPCL file printing");
	...
	if (fd < 0) {
		papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
			    "Failed to open job file: %s", strerror(errno));
		return false;
	}
	...
	while ((bytes_read = read(fd, &ch, 1)) > 0) {
		...
		if (line_pos < sizeof(line) - 1) {
			...
			papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
				    "Line too long (exceeds %lu bytes)",
			...
			if (line_pos > 1 && line[0] != '#') {
				...
				if (command_count % 10 == 0) {
					papplLogJob(
						...
						"Sent %u TPCL commands to printer",
						command_count);
				}
			} else if (line[0] == '#') {
				papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
					    "Skipping comment: %s", line);
			}
	...
	if (bytes_read < 0) {
		papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
			    "Error reading job file: %s", strerror(errno));
		close(fd);
		return false;
	}
	...
	if (line_pos > 0) {
		...
		if (line[0] != '#') {
			papplLogJob(
				job, PAPPL_LOGLEVEL_WARN,
				"Last line missing newline terminator, sending anyway: %s",
				line);
			papplDeviceWrite(device, line, line_pos);
			command_count++;
		}
	...
	papplLogJob(job, PAPPL_LOGLEVEL_INFO,
		    "Raw TPCL file printing completed: %u commands sent",
		    command_count);
		       ...
		       pappl_device_t *device)
{
	...
	if (!tpcl_job) {
		papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
			    "Failed to allocate job data structure");
		ippDelete(printer_attrs);
		return false;
	}
	...
	const char *graphics_mode = tpcl_get_str_option(
		printer_attrs, "graphics-mode", "topix", job, NULL);
	...
	if (strcmp(graphics_mode, "nibble-and") == 0)
		...
		papplLogJob(job, PAPPL_LOGLEVEL_WARN,
			    "Unknown graphics mode '%s', defaulting to TOPIX",
			    graphics_mode);
		tpcl_job->gmode = TEC_GMODE_TOPIX;
	}
	papplLogJob(job, PAPPL_LOGLEVEL_DEBUG, "Graphics mode set to: %s (%d)",
		    graphics_mode, tpcl_job->gmode);
	...
	if (tpcl_job->gmode == TEC_GMODE_TOPIX) {
		...
		if ((options->header.HWResolution[0] != 150 &&
		     options->header.HWResolution[0] != 203 &&
		     options->header.HWResolution[0] != 300) ||
		    (options->header.HWResolution[1] != 150 &&
		     options->header.HWResolution[1] != 203 &&
		     options->header.HWResolution[1] != 300) ||
		    (options->header.HWResolution[0] !=
		     options->header.HWResolution[1])) {
			papplLogJob(
				job, PAPPL_LOGLEVEL_ERROR,
				"TOPIX mode only supports 150x150, 203x203, or 300x300 dpi resolution. Requested: %ux%u dpi",
				options->header.HWResolution[0],
				options->header.HWResolution[1]);
		...
		tpcl_job->compbuf = tpcl_compbuf_create(
			options->header.cupsBytesPerLine, job);

		if (!tpcl_job->buffer || !tpcl_job->compbuf) {
			papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
				    "Failed to allocate TOPIX buffers");
			tpcl_free_job_buffers(job, tpcl_job);
			ippDelete(printer_attrs);
			return false;
		}
		...
		if (!tpcl_job->buffer) {
			papplLogJob(
				job, PAPPL_LOGLEVEL_ERROR,
				"Failed to allocate line buffer for HEX / Nibble mode");
			tpcl_free_job_buffers(job, tpcl_job);
			ippDelete(printer_attrs);
			return false;
		}
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "HEX mode buffer allocated: line=%u bytes",
	...
	tpcl_job->print_width =
		(int)(options->header.cupsPageSize[0] * MM_PER_INCH * 10.0 /
			      POINTS_PER_INCH +
		      0.5); // Effective print width (0.1mm)
	tpcl_job->print_height =
		(int)(options->header.cupsPageSize[1] * MM_PER_INCH * 10.0 /
			      POINTS_PER_INCH +
		      0.5); // Effective print height (0.1mm)

	// Get label gap and roll margin from printer settings
	int label_gap =
		tpcl_get_int_option(printer_attrs, "label-gap", 50, job, NULL);
	int roll_margin = tpcl_get_int_option(printer_attrs, "roll-margin", 10,
					      job, NULL);
	...
	papplLogJob(
		...
		tpcl_job->print_width, tpcl_job->print_height,
		tpcl_job->label_pitch, tpcl_job->roll_width);
	papplLogJob(
		job, PAPPL_LOGLEVEL_DEBUG,
		"Maximum image resolution at %ux%udpi: width=%u dots, height=%u dots",
		options->header.HWResolution[0],
		options->header.HWResolution[1],
		(unsigned int)(options->header.HWResolution[0] *
			       options->header.cupsPageSize[0] /
			       POINTS_PER_INCH),
		(unsigned int)(options->header.HWResolution[1] *
	...
	if (options->header.cupsBitsPerPixel == 0) {
		papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
			    "Invalid raster data: cupsBitsPerPixel is 0");
		tpcl_free_job_buffers(job, tpcl_job);
		ippDelete(printer_attrs);
		return false;
	}
	...
	papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
		    "Calculated buffer_len=%zu bytes (for %u pixels)",
		    tpcl_job->buffer_len, options->header.cupsWidth);
	...
	if (tpcl_job->label_pitch > max_pitch) {
		papplLogJob(
			job, PAPPL_LOGLEVEL_ERROR,
			"Label pitch %d (0.1mm) exceeds maximum %d (0.1mm) for %udpi resolution",
			tpcl_job->label_pitch, max_pitch,
			options->header.HWResolution[1]);
		tpcl_free_job_buffers(job, tpcl_job);
		ippDelete(printer_attrs);
		return false;
	...
	if (tpcl_job->print_height > max_height) {
		papplLogJob(
			job, PAPPL_LOGLEVEL_ERROR,
			"Print height %d (0.1mm) exceeds maximum %d (0.1mm) for %udpi resolution",
			tpcl_job->print_height, max_height,
			options->header.HWResolution[1]);
		tpcl_free_job_buffers(job, tpcl_job);
		ippDelete(printer_attrs);
		return false;
	...
	// Send label size command
	tpcl_cmd_label_size(device, tpcl_job->label_pitch,
			    tpcl_job->print_width, tpcl_job->print_height,
			    tpcl_job->roll_width, job, NULL);
	...
	bool has_adjustments = tpcl_get_feed_adjustments(
		printer_attrs, &feed_adjustment, &cut_position_adjustment,
		&backfeed_adjustment, job, NULL);
	...
	if (has_adjustments) {
		...
		tpcl_cmd_position_adjust(device, feed_adjustment,
					 cut_position_adjustment,
					 backfeed_adjustment, job, NULL);
	} else {
		papplLogJob(
			job, PAPPL_LOGLEVEL_DEBUG,
			"Skipping AX command - all adjustment values are 0");
	}

	// Print density adjustment command - only send when print-darkness is not 0
	// Get print darkness from printer settings (-10 to 10)
	int print_darkness = tpcl_get_int_option(
		printer_attrs, "print-darkness", 0, job, NULL);
	...
	if (print_darkness != 0) {
		...
		if (!papplPrinterGetDriverData(printer, &driver_data)) {
			papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
				    "Failed to get driver data for AY command");
			tpcl_free_job_buffers(job, tpcl_job);
			ippDelete(printer_attrs);
			return false;
		} else {
			...
			const char *media_type = driver_data.media_default.type;
			if (media_type) {
				if (strncmp(media_type, "thermal-transfer",
					    16) == 0)
					mode_char = '0';
				papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
					    "Media type: %s, AY mode: %c",
					    media_type, mode_char);
			}

			tpcl_cmd_darkness_adjust(device, print_darkness,
						 mode_char, job, NULL);
		}
	} else {
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "Skipping AY command - print darkness is 0");
	}

	// Check if label dimensions have changed and update state file
	bool label_size_changed = tpcl_state_check_and_update(
		printer, tpcl_job->print_width, tpcl_job->print_height,
		label_gap, roll_margin, job);

	// If label size changed and feed-on-label-size-change is enabled, send feed command
	const char *feed_on_change_str = tpcl_get_str_option(
		printer_attrs, "feed-on-label-size-change", "no", job, NULL);
	bool should_feed = label_size_changed &&
			   (strcmp(feed_on_change_str, "yes") == 0);

	if (should_feed) {
		papplLogJob(
			job, PAPPL_LOGLEVEL_DEBUG,
			"Label size changed and feed-on-label-size-change is enabled, sending feed command");
		...
		if (!papplPrinterGetDriverData(printer, &driver_data)) {
			...
			const char *media_type = driver_data.media_default.type;
			if (media_type) {
				if (strcmp(media_type,
					   "thermal-transfer-ribbon-saving") ==
				    0)
					ribbon_char = '1';
				else if (strcmp(media_type,
						"thermal-transfer-no-ribbon-saving") ==
					 0)
					ribbon_char = '2';
			}

			tpcl_cmd_feed(device, sensor_char, cut_char,
				      feed_mode_char, speed_char, ribbon_char,
				      job, NULL);
		}
	}
			...
			pappl_device_t *device, unsigned page)
{
	...
	if (!tpcl_job) {
		papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
			    "Job data structure not initialized");
		return false;
	}
			...
			const unsigned char *line)
{
	if (y == 0) {
		papplLogJob(
			job, PAPPL_LOGLEVEL_DEBUG,
			"Starting line %u (logging debug messages for the first and last 3 lines only)",
			y);
	}
	...
	if (!tpcl_job || !tpcl_job->buffer) {
		papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
			    "Line %u: Job data or buffer not initialized", y);
		return false;
	}
	...
	if (options->header.cupsBitsPerPixel == 8) {
		if ((y < 3) | (y > (options->header.cupsHeight - 4))) {
			papplLogJob(
				job, PAPPL_LOGLEVEL_DEBUG,
				"Line %u: Using 8 bit to 1 bit dithering for image output",
				y);
		}
		...
		for (unsigned int x = 0; x < options->header.cupsBytesPerLine;
		     x++) {
			if (line[x] >=
			    options->dither
				    [y & 15]
				    [x &
				     15]) // If pixel is above threshold, set bit to 1
			{
				tpcl_job->buffer[x / 8] |=
					(0x80 >> (x & 7)); // Set bit MSB-first
			}
		}
	}
	// For 1bit color depth (1 byte = 8 pixels, black and white), just copy to target buffer
		...
		if ((y < 3) | (y > (options->header.cupsHeight - 4))) {
			papplLogJob(
				job, PAPPL_LOGLEVEL_DEBUG,
				"Line %u: Using native 1 bit color depth for image output",
				y);
		}

		// Move data to target buffer
		memcpy(tpcl_job->buffer, line,
		       options->header.cupsBytesPerLine);
	} else {
		papplLogJob(
			job, PAPPL_LOGLEVEL_ERROR,
			"Line %u: Only 1 bit or 8 bit color depths are supported, request was for %u bit",
			y, options->header.cupsBitsPerPixel);
		tpcl_free_job_buffers(job, tpcl_job);
		return false;
	}

	// Determine if the print job is black ink plane (1 = black) or white ink plane (1 = white). Printer expects black ink plane (1 = black)
	if (options->header.cupsColorSpace ==
	    CUPS_CSPACE_SW) // 1 = white -> invert all bits
	{
		if ((y < 3) | (y > (options->header.cupsHeight - 4))) {
			papplLogJob(
				job, PAPPL_LOGLEVEL_DEBUG,
				"Line %u: Flipping bits to translate from white ink plane (1 = white) to black ink plane (1 = black)",
				y);
		}

		for (unsigned int x = 0; x < tpcl_job->buffer_len; x++) {
			tpcl_job->buffer[x] =
				(unsigned char)~tpcl_job->buffer[x];
		}
	} else if (options->header.cupsColorSpace != CUPS_CSPACE_K) {
		papplLogJob(
			job, PAPPL_LOGLEVEL_ERROR,
			"Line %u: Only K(3) and SW(18) color spaces supported, request was for space (%d)",
			y, options->header.cupsColorSpace);
		tpcl_free_job_buffers(job, tpcl_job);
		return false;
	}

	// Determine the transmission mode
	if ((tpcl_job->gmode == TEC_GMODE_HEX_AND) |
	    (tpcl_job->gmode == TEC_GMODE_HEX_OR)) {
		if ((y < 3) | (y > (options->header.cupsHeight - 4))) {
			papplLogJob(
				job, PAPPL_LOGLEVEL_DEBUG,
				"Line %u: Transmitting %lu bytes in hex mode",
				y, (unsigned long)tpcl_job->buffer_len);
		}

		papplDeviceWrite(device, tpcl_job->buffer,
				 tpcl_job->buffer_len);
	} else if ((tpcl_job->gmode == TEC_GMODE_NIBBLE_AND) |
		   (tpcl_job->gmode == TEC_GMODE_NIBBLE_OR)) {
		...
		if ((y < 3) | (y > (options->header.cupsHeight - 4))) {
			papplLogJob(
				job, PAPPL_LOGLEVEL_DEBUG,
				"Line %u: Transmitting %lu bytes in nibble mode (ASCII mode)",
				y, (unsigned long)(tpcl_job->buffer_len * 2));
		}

		// Debug output: allocate buffer for ASCII representation file dump
		char *debug_buffer = NULL;
		if (papplSystemGetLogLevel(papplPrinterGetSystem(
			    papplJobGetPrinter(job))) >= PAPPL_LOGLEVEL_DEBUG) {
		...
		for (unsigned int x = 0; x < tpcl_job->buffer_len; x++) {
			...
			unsigned char out[2] = {
				(unsigned char)(0x30 |
						((tpcl_job->buffer[x] >> 4) &
						 0x0F)), // high nibble to low nibble
				(unsigned char)(0x30 |
						(tpcl_job->buffer[x] &
						 0x0F)) // low nibble to low nibble
			};

			// Store ASCII representation in debug buffer
			if (debug_buffer) {
				debug_buffer[x * 2] =
					(out[0] >= 0x30 && out[0] <= 0x3F) ?
						out[0] :
						'.';
				debug_buffer[x * 2 + 1] =
					(out[1] >= 0x30 && out[1] <= 0x3F) ?
						out[1] :
						'.';
			}
		...
		if (debug_buffer) {
			...
			snprintf(filename, sizeof(filename),
				 "/tmp/rastertotpcl-nibble-dump-%d.out",
				 getpid());
			...
			if (fp) {
				...
				if (y == options->header.cupsHeight - 1) {
					papplLogJob(
						job, PAPPL_LOGLEVEL_INFO,
						"Dump file with image data written to %s",
						filename);
				}
			} else {
				papplLogJob(
					job, PAPPL_LOGLEVEL_ERROR,
					"Failed to open debug dump file: %s",
					filename);
			}
		...
		if ((y < 3) | (y > (options->header.cupsHeight - 4))) {
			papplLogJob(
				job, PAPPL_LOGLEVEL_DEBUG,
				"Line %u: Compressing %lu bytes in TOPIX mode",
				y, (unsigned long)tpcl_job->buffer_len);
		}
		...
		size_t buffer_used =
			tpcl_topix_get_buffer_used(tpcl_job->compbuf);
		size_t buffer_threshold = TPCL_COMP_BUFFER_MAX -
					  (tpcl_job->buffer_len +
					   ((tpcl_job->buffer_len / 8) * 3));

		if ((buffer_used > buffer_threshold) |
		    (y == (options->header.cupsHeight - 1))) {
			papplLogJob(
				job, PAPPL_LOGLEVEL_DEBUG,
				"Line %u: TOPIX buffer full (%lu/65535 bytes) or last line, flushing. Y offset for this image: %d (0.1mm)",
				y, (unsigned long)buffer_used,
				tpcl_job->y_offset);

			ssize_t bytes_written = tpcl_topix_flush(
				tpcl_job->compbuf, device, tpcl_job->y_offset,
				options->header.cupsWidth,
				options->header.HWResolution[0],
				tpcl_job->gmode);

			// Y offset for next image in 0.1mm (for TOPIX)
			tpcl_job->y_offset = (int)(y + 1) * MM_PER_INCH * 10.0 /
					     options->header.HWResolution[0];

			papplLogJob(
				job, PAPPL_LOGLEVEL_DEBUG,
				"Line %u: TOPIX buffer flushed, %ld bytes sent. Y offset for next image: %d (0.1mm)",
				y, (long)bytes_written, tpcl_job->y_offset);
		}
	} else {
		papplLogJob(
			job, PAPPL_LOGLEVEL_ERROR,
			"Line %u: Graphics transmission mode %d not supported",
			y, tpcl_job->gmode);
		tpcl_free_job_buffers(job, tpcl_job);
		return false;
	}
		      ...
		      pappl_device_t *device, unsigned page)
{
	...
	if (tpcl_job->gmode != TEC_GMODE_TOPIX) {
		...
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
			    "Closing HEX graphics data with: |}}");
	}
	...
	if (!papplPrinterGetDriverData(printer, &driver_data)) {
		papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
			    "Failed to get driver data for XS command");
		ippDelete(printer_attrs);
		tpcl_free_job_buffers(job, tpcl_job);
		return false;
	}
	...
	if (num_copies < 1 || num_copies > 9999) {
		papplLogJob(
			job, PAPPL_LOGLEVEL_ERROR,
			"Invalid number of copies %d, must be in range [1-9999]",
			num_copies);
		ippDelete(printer_attrs);
		tpcl_free_job_buffers(job, tpcl_job);
		return false;
	...
	int cut_interval = tpcl_get_int_option(printer_attrs, "cut-interval", 0,
					       job, NULL);
	if (cut_interval < 0 || cut_interval > 100) {
		papplLogJob(job, PAPPL_LOGLEVEL_ERROR,
			    "Invalid cut interval %d, must be in range [0-100]",
			    cut_interval);
		ippDelete(printer_attrs);
		tpcl_free_job_buffers(job, tpcl_job);
		return false;
	...
	const char *sensor_type = tpcl_get_str_option(
		printer_attrs, "sensor-type", "transmissive", job, NULL);
	char sensor_char = tpcl_map_sensor_type(sensor_type);
	...
	int print_speed =
		tpcl_get_int_option(printer_attrs, "print-speed", 3, job, NULL);
	char speed_char = '0' + print_speed;
	if (print_speed > 9)
		speed_char = 'A' + (print_speed - 10);
	...
	// Send the XS command
	tpcl_cmd_issue_label(device, num_copies, cut_interval, sensor_char,
			     feed_mode_char, speed_char, ribbon_char,
			     rotation_char, status_response_char, job, NULL);
	...
	if (device_uri && strncmp(device_uri, "socket://", 9) == 0) {
		papplLogJob(
			job, PAPPL_LOGLEVEL_DEBUG,
			"Sending 1024 space padding (TCP workaround for network connection)");
		     ...
		     pappl_device_t *device)
{
	...
	if (printer) {
		...
		if (driver_name && strstr(driver_name, "B-EV4T")) {
			static unsigned char dummy_data[600] = {
				0
			}; // 600 null bytes for BEV4T workaround
			papplLogJob(
				job, PAPPL_LOGLEVEL_DEBUG,
				"Sending 600 null bytes (BEV4T workaround)");
			papplDeviceWrite(device, dummy_data,
					 sizeof(dummy_data));
			papplDeviceFlush(device);
		}
	}
	...
	tpcl_free_job_buffers(job, tpcl_job);
	papplLogJob(job, PAPPL_LOGLEVEL_DEBUG,
		    "Freeing page buffers and job data structure");
			     ...
			     size_t bufsize)
{
	...
	if (!device) {
		papplLogPrinter(
			printer, PAPPL_LOGLEVEL_ERROR,
			"Failed to open device connection for test page");
		return NULL;
	}
	...
	bool has_adjustments = tpcl_get_feed_adjustments(
		printer_attrs, &feed_adjustment, &cut_position_adjustment,
		&backfeed_adjustment, NULL, printer);
	...
	int print_darkness = tpcl_get_int_option(
		printer_attrs, "print-darkness", 0, NULL, printer);
	...
	if (print_darkness != 0) {
		...
		tpcl_cmd_darkness_adjust(device, print_darkness, mode_char,
					 NULL, printer);
	} else {
		papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
				"Skipping AY command - print darkness is 0");
	...
	bool label_size_changed = tpcl_state_check_and_update(
		printer, print_width, print_height, label_pitch - print_height,
		roll_width - print_width, NULL);

	// Check if feed-on-label-size-change is enabled
	const char *feed_on_change_str =
		tpcl_get_str_option(printer_attrs, "feed-on-label-size-change",
				    "no", NULL, printer);
	bool should_feed = label_size_changed &&
			   (strcmp(feed_on_change_str, "yes") == 0);
	...
	if (should_feed) {
		...
		const char *sensor_type =
			tpcl_get_str_option(printer_attrs, "sensor-type",
					    "transmissive", NULL, printer);
		char sensor_char = tpcl_map_sensor_type(sensor_type);

		// b: Cut selection (0=non-cut, 1=cut)
		const char *cut_type = tpcl_get_str_option(
			printer_attrs, "label-cut", "non-cut", NULL, printer);
		char cut_char = tpcl_map_cut_type(cut_type);

		// c: Feed mode (C=batch, D=strip with backfeed sensor valid, E=strip with backfeed sensor ignored, F=partial-cut)
		const char *feed_mode = tpcl_get_str_option(
			printer_attrs, "feed-mode", "batch", NULL, printer);
		char feed_mode_char = tpcl_map_feed_mode(feed_mode);

		// d: Feed speed (retrieve from vendor option with fallback to 3)
		int print_speed = tpcl_get_int_option(
			printer_attrs, "print-speed", 3, NULL, printer);
		char speed_char = '0' + print_speed;
		if (print_speed > 9)
			speed_char = 'A' + (print_speed - 10);
		...
		if (media_type) {
			if (strcmp(media_type,
				   "thermal-transfer-ribbon-saving") == 0)
				ribbon_char = '1';
			else if (strcmp(media_type,
					"thermal-transfer-no-ribbon-saving") ==
				 0)
		...
		tpcl_cmd_feed(device, sensor_char, cut_char, feed_mode_char,
			      speed_char, ribbon_char, NULL, printer);
	}
	...
	papplLogPrinter(
		printer, PAPPL_LOGLEVEL_DEBUG,
		"Drawing concentric boxes: spacing=%d, line_width_dots=%d",
		box_spacing, line_width_dots);
	...
	for (int offset = 0;; offset += box_spacing, box_num++) {
		...
		if (box_width < min_dimension || box_height < min_dimension) {
			papplLogPrinter(
				printer, PAPPL_LOGLEVEL_DEBUG,
				"Stopping box drawing: box %d would be %dx%d (min=%d)",
		...
		tpcl_cmd_line(device, x1, y1, x2, y2, 1, line_width_dots, NULL,
			      printer);
	}
	...
	tpcl_cmd_issue_label(device, num_copies, cut_interval, sensor_char,
			     feed_mode_char, speed_char, ribbon_char,
			     rotation_char, status_response_char, NULL,
			     printer);
...
void tpcl_delete_cb(pappl_printer_t *printer, pappl_pr_driver_data_t *data)
{
	...
	papplLogPrinter(printer, PAPPL_LOGLEVEL_INFO,
			"Printer deleted, cleaning up resources");
...
static void tpcl_free_job_buffers(pappl_job_t *job, tpcl_job_t *tpcl_job)
{
	if (tpcl_job) {
		...
		if (tpcl_job->compbuf)
			tpcl_compbuf_delete(tpcl_job->compbuf);
		free(tpcl_job);
	}
