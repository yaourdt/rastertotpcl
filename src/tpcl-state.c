...
static int open_state_file(pappl_printer_t *printer, const char *mode,
			   char *filepath, size_t filepath_size);
static bool load_state_from_file(pappl_printer_t *printer,
				 tpcl_printer_state_t *state);
static bool save_state_to_file(pappl_printer_t *printer,
			       const tpcl_printer_state_t *state);
static bool state_has_changed(const tpcl_printer_state_t *state, int w, int h,
			      int g, int m);
static void log_state_change(const tpcl_printer_state_t *old, int w, int h,
			     int g, int m, pappl_job_t *job,
	...
	if (!had_state) {
		...
		if (job)
			papplLogJob(
				...
				"No previous label dimensions found, this is likely the first job");
		else
			papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG,
					"No previous label dimensions found");
	} else {
		// Check if dimensions changed
		changed = state_has_changed(&state, print_width, print_height,
					    label_gap, roll_margin);

		if (changed) {
			log_state_change(&state, print_width, print_height,
					 label_gap, roll_margin, job, printer);
		}
	}
...
void tpcl_state_delete(pappl_printer_t *printer)
{
	...
	papplLogPrinter(
		printer, fd >= 0 ? PAPPL_LOGLEVEL_INFO : PAPPL_LOGLEVEL_DEBUG,
			   ...
			   char *filepath, size_t filepath_size)
{
	...
	fd = papplPrinterOpenFile(printer, path, sizeof(path), NULL,
				  "labelstate", "state", mode);
				 ...
				 tpcl_printer_state_t *state)
{
	...
	FILE *fp;
	char line[PATH_MAX];
	int loaded_width = -1, loaded_height = -1, loaded_gap = -1,
	    loaded_margin = -1;
	...
	if (loaded_width < 0 || loaded_height < 0 || loaded_gap < 0 ||
	    loaded_margin < 0) {
		papplLogPrinter(printer, PAPPL_LOGLEVEL_WARN,
				"Failed to load complete state file, ignoring");
		return false;
	}
	...
	papplLogPrinter(
		...
		"Loaded state from %s: width=%d, height=%d, gap=%d, margin=%d",
		filepath, loaded_width, loaded_height, loaded_gap,
		loaded_margin);
			       ...
			       const tpcl_printer_state_t *state)
{
	...
	papplLogPrinter(
		printer, PAPPL_LOGLEVEL_DEBUG,
		"Saved state to %s: width=%d, height=%d, gap=%d, margin=%d",
		filepath, state->last_print_width, state->last_print_height,
...
static bool state_has_changed(const tpcl_printer_state_t *state, int w, int h,
			      int g, int m)
{
	return (state->last_print_width != w || state->last_print_height != h ||
		state->last_label_gap != g || state->last_roll_margin != m);
}
...
{
	const char *fmt =
		"Label size changed: old(%d×%d, %d, %d) → new(%d×%d, %d, %d) [width×height, gap, margin in 0.1mm]";

	if (job)
		papplLogJob(job, PAPPL_LOGLEVEL_DEBUG, fmt,
			    old->last_print_width, old->last_print_height,
			    old->last_label_gap, old->last_roll_margin, w, h, g,
			    m);
	else if (printer)
		papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG, fmt,
				old->last_print_width, old->last_print_height,
				old->last_label_gap, old->last_roll_margin, w,
				h, g, m);
}
